...
# 整个脚本最为重要的部分。首先设置 DJANGO_SETTINGS_MODULE 环境变量，这将指定 django 启动时使用的配置文件，
# 然后运行 django.setup() 启动 django。
...
if __name__ == '__main__':
    ...
    print(Category.objects.create(name='Markdown测试'))

    Post.objects.create(
        title='Markdown 与代码高亮测试',
        # body=pathlib.Path(BASE_DIR).joinpath('scripts', 'md.sample').read_text(),
        body='Markdown 与代',
        # body=pathlib.Path(BASE_DIR),
        category=Category.objects.create(name='Markdown测试'),
        author=user,
    )
    # 这个脚本没什么说的简单地使用 django 的 ORM API 生成博客用户、分类、标签以及一篇 Markdown 测试文章
    ...
    #     post.save()
        ...
        # 最后依葫芦画瓢，给前 20 篇文章(Post) 生成评论数据
        # 要注意的是评论的发布时间必须位于被评论文章的发布时间和当前时间之间，
        # 这就是delta_in_days = '-' + str((timezone.now() - post_created_time).days) + 'd'这句代码的作用
