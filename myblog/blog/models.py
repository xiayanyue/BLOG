...
# 每次保存模型时都应该修改 modified_time 的值每一个 Model 都有一个 save 方法，
# 这个方法包含了将 model 数据保存到数据库中的逻辑通过覆写这个方法在 model 被 save 到数据库前指定 modified_time 的值为当前时间
    def save(self,*args,**kwargs):
        self.modified_time = timezone
        ...
        super().save(*args,**kwargs) #注意在指定完 modified_time 的值后，调用父类的 save 以执行数据保存回数据库的逻辑


    # 自定义 get_absolute_url 方法33
